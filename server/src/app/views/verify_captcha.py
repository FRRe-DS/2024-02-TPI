...
class VerifyCaptchaView(APIView):
    def post(self, request):
        ...
        if not turnstile_response:
            return Response(
                {"error": "CAPTCHA no completado"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if result.get("success"):
            ...
            return Response(
                {"success": "CAPTCHA verificado correctamente"},
                status=status.HTTP_200_OK,
            )
        else:
            # Si la validación falló
            return Response(
                {"error": "CAPTCHA inválido"}, status=status.HTTP_400_BAD_REQUEST
            )
